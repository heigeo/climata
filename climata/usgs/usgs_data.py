...
from climata.usgs.__init__ import *
...
def load_data(basin):
    ...
    parms = ParamIO() # This is a lookup dictionary for 
    
    sys.stdout.write('"'+'","'.join(summary_header_fields)+'"')
    sys.stdout.write("\n")
    for row in sites:
        parameter_code = [row.parm_cd]
        if row.agency_cd != 'USGS': # The agency code is 5s on the first row. This should skip that row...hopefully
            pass
        else:
            ...
            if row.parm_cd == '' or row.parm_cd == '00000' or row.parm_cd == 0 or row.parm_cd == None:
                prmc = 'N/A'
            else:
                prmc = parms[row.parm_cd].group + ': ' + parms[row.parm_cd].parm_nm
            rowprint = (
                row.agency_cd,
                row.site_no,
                row.station_nm,
